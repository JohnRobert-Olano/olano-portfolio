...
import { Briefcase, GraduationCap, ChevronDown, ChevronUp, Calendar, Star, Sparkles } from "lucide-react";
...
function TimelineNode({
  ...
  const nodeVariants = {
    below: { opacity: 0, y: 30, rotateX: 6, transformPerspective: 1000, scale: 0.98 },
    inside: { opacity: 1, y: 0, rotateX: 0, transformPerspective: 1000, scale: 1 },
    above: { opacity: 0, y: -30, rotateX: -6, transformPerspective: 1000, scale: 0.98 },
  };
      ...
      <div 
        className={`absolute -left-[35px] md:-left-[54px] top-1.5 w-5 h-5 md:w-7 md:h-7 rounded-none flex items-center justify-center border transition-all duration-300 ${
          isExpanded 
            ...
            : "bg-[#050505] border-zinc-800 text-zinc-500"
        }`}
      >
        {isAcademic ? (
        ...
        <div className="mt-5 pt-3 border-t border-zinc-900/50 flex items-center justify-between text-[10px] text-zinc-500 font-mono uppercase font-bold">
          ...
          <div className="p-1 rounded-sm bg-zinc-900 border border-zinc-800">
            {isExpanded ? (
              ...
              <ChevronDown className="w-4 h-4 text-zinc-505" />
            )}
          </div>
        </div>
        ...
        <AnimatePresence>
          {isExpanded && (
            <motion.div
              ...
              className="overflow-hidden mt-6 pt-6 border-t border-zinc-900 space-y-6"
            >
              ...
              <div className="space-y-3">
                ...
                <div className="flex flex-wrap gap-2">
                  {node.skills.map((skill) => (
                    <span 
                      key={skill} 
                      className="text-[9px] font-mono tracking-wider bg-zinc-900 border border-zinc-800 px-2.5 py-1 rounded-none text-zinc-300 font-bold uppercase"
                    >
                      {skill}
                    </span>
                  ))}
...
export default function Experience() {
  ...
  return (
    <section id="experience" className="relative py-24 px-6 lg:px-24 bg-[#050505] border-t border-zinc-900 tech-grid-bg scroll-mt-24">
